...
class PlannerAgent:
    ...
    def _deterministic_plan(self, topic: str, depth_level: int = 2) -> ResearchPlan:
        domain_plan = self.domain_pack.deterministic_plan(topic, depth_level)
    ...
    def _llm_plan(self, topic: str, depth_level: int, research_id: str) -> ResearchPlan:
        ...
        plan = self.domain_pack.propagate_plan_identity(plan, topic)
        self.last_stats = {
            "fallback": False,
    ...
    def _valid_structured_requests(
        ...
        return [
            ...
            if self.domain_pack.valid_structured_request(request)
        ]
