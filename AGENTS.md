...
当前仓库处于 MVP 阶段：已实现确定性的 Planner、Researcher、Extractor、Critic、Reporter、Evaluator 工作流；默认使用本地 fixture 检索数据；Evidence、checkpoint 和 evaluation 结果通过 SQLite 存储；CLI demo 和 unittest 套件已在本地 `.venv` 验证通过。当前执行路径仍是项目自研同步 engine，尚未迁移为 LangGraph runtime graph。
...
- Streamlit：`>=1.35`。
- LangGraph：`pyproject.toml` 声明 `langgraph>=0.2.50`；本地 `.venv` 实际安装版本为 1.2.2。当前工作流代码未直接使用 LangGraph 图执行，且本地环境未安装官方 SQLite checkpointer 包。
- LiteLLM：`>=1.40`，列在依赖中；当前代码未发现实际 LiteLLM 调用或统一 LLM 封装，下一阶段启用前仍视为未接入。
...
```bash
PYTHONPATH=src PYTHONDONTWRITEBYTECODE=1 DEEPRESEARCH_SEARCH_PROVIDER=fixture DEEPRESEARCH_STORAGE_PATH=_collab/002_langgraph-migration/verification/test_research.db .venv/bin/python -m unittest discover -s tests
```

002 已验证可用的确定性 demo 命令：

```bash
PYTHONPATH=src PYTHONDONTWRITEBYTECODE=1 DEEPRESEARCH_SEARCH_PROVIDER=fixture DEEPRESEARCH_STORAGE_PATH=_collab/002_langgraph-migration/verification/research.db .venv/bin/python scripts/run_demo.py --topic 'AI Agent 在财富管理行业的落地机会研究' --depth 1 --output _collab/002_langgraph-migration/verification/report.md
```

未在 002 任务验证的命令：`scripts/run_eval.py`、`scripts/run_checkpoint_demo.py`、`scripts/dev_server.py`、FastAPI/Uvicorn 入口、Streamlit 入口、Docker/Compose 入口。
...
- 所有 LLM 调用必须经统一封装层并记录 token、cost、latency。目标规范，现状未满足：当前未发现实际 LLM 调用，也未发现统一 LLM 封装层；工作流中的 token/cost 为确定性估算累加。
