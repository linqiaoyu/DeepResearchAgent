...
当前仓库处于 MVP 阶段：已实现确定性的 Planner、Researcher、Extractor、Critic、Reporter、Evaluator 工作流；默认使用本地 fixture 检索数据；编排层已迁移为 LangGraph `StateGraph`，Researcher 按子问题 fan-out，Critic 通过条件边回流 retry queue；checkpoint 由官方 `SqliteSaver` 写入 SQLite，Evidence 和 evaluation 结果由 `SQLiteStore` 写入 SQLite；LLM 模式通过统一 LiteLLM 层覆盖 Planner、Extractor、Reporter，Researcher 与 Critic 当前仍保持确定性；CLI demo 和 unittest 套件已在本地 `.venv` 验证通过。
...
- `data/`：评测集、bad cases、mock source fixture 和 runtime 数据。
- `docs/`：架构、评估、部署、provider 集成和面试问答文档。
- `prompts/`：当前存在 `critic.md`、`planner.md`、`reporter.md`。
- `scripts/`：`run_demo.py`、`run_eval.py`、`run_checkpoint_demo.py`、`dev_server.py`。
...
```bash
PYTHONPATH=src PYTHONDONTWRITEBYTECODE=1 DEEPRESEARCH_SEARCH_PROVIDER=fixture DEEPRESEARCH_MODE=deterministic DEEPRESEARCH_STORAGE_PATH=_collab/004_llm-integration/verification/test_research_final.db .venv/bin/python -m unittest discover -s tests
```

004 已验证可用的确定性 demo 命令：

```bash
PYTHONPATH=src PYTHONDONTWRITEBYTECODE=1 DEEPRESEARCH_SEARCH_PROVIDER=fixture DEEPRESEARCH_MODE=deterministic DEEPRESEARCH_STORAGE_PATH=_collab/004_llm-integration/comparison/deterministic.db .venv/bin/python scripts/run_demo.py --mode deterministic --topic 'AI Agent 在财富管理行业的落地机会研究' --depth 1 --output _collab/004_llm-integration/comparison/deterministic_report.md
```

004 已验证可用的 LLM smoke 命令（需要 `.env` 中存在 `DEEPSEEK_API_KEY`，fixture 检索）：

```bash
PYTHONPATH=src PYTHONDONTWRITEBYTECODE=1 DEEPRESEARCH_SEARCH_PROVIDER=fixture DEEPRESEARCH_MODE=llm DEEPRESEARCH_STORAGE_PATH=_collab/004_llm-integration/smoke/run1/research.db DEEPRESEARCH_LLM_LEDGER_PATH=_collab/004_llm-integration/smoke/llm_ledger.jsonl .venv/bin/python scripts/run_demo.py --mode llm --topic 'AI Agent 在财富管理行业的落地机会研究' --depth 1 --output _collab/004_llm-integration/smoke/run1/report.md
```

004 额外验证：`PYTHONPATH=src .venv/bin/python -m ruff check src tests scripts`。

未在 004 任务验证的命令：`scripts/run_eval.py`、`scripts/run_checkpoint_demo.py`、`scripts/dev_server.py`、FastAPI/Uvicorn 入口、Streamlit 入口、Docker/Compose 入口。
...
- 所有 LLM 调用必须经统一封装层并记录 token、cost、latency。当前 `LLMClient` 已记录 token、cost、latency、cache_hit、repair_attempts 到 `data/runtime/llm_ledger.jsonl`；deterministic 模式继续使用估算值。
