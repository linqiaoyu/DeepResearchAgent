...
- `data/`：评测集、Golden Set v1、bad cases、mock source fixture 和 runtime 数据。
...
未在 005/006R3 任务验证的命令：`scripts/run_checkpoint_demo.py`、`scripts/dev_server.py`、FastAPI/Uvicorn 入口、Streamlit 入口、Docker/Compose 入口。
...
- `data/eval_set_deterministic.jsonl`、`data/eval_baseline.json`、`data/golden_set/v1/`、`data/bad_cases_deterministic.jsonl`、`data/bad_cases_llm.jsonl`、`data/mock_data/` 是受管资产，必须保持追踪。
