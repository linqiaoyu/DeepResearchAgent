...
```bash
PYTHONPATH=src PYTHONDONTWRITEBYTECODE=1 DEEPRESEARCH_SEARCH_PROVIDER=fixture DEEPRESEARCH_STORAGE_PATH=_collab/003_langgraph-migration/verification/test_research.db .venv/bin/python -m unittest discover -s tests
```

003 已验证可用的确定性 demo 命令：

```bash
PYTHONPATH=src PYTHONDONTWRITEBYTECODE=1 DEEPRESEARCH_SEARCH_PROVIDER=fixture DEEPRESEARCH_STORAGE_PATH=_collab/003_langgraph-migration/verification/research.db .venv/bin/python scripts/run_demo.py --topic 'AI Agent 在财富管理行业的落地机会研究' --depth 1 --output _collab/003_langgraph-migration/verification/report.md
```

003 额外验证：`PYTHONPATH=src .venv/bin/python -m ruff check src tests`。

未在 003 任务验证的命令：`scripts/run_eval.py`、`scripts/run_checkpoint_demo.py`、`scripts/dev_server.py`、FastAPI/Uvicorn 入口、Streamlit 入口、Docker/Compose 入口。
