...
当前仓库处于 MVP 阶段：已实现确定性的 Planner、Researcher、Extractor、Critic、Reporter、Evaluator 工作流；默认使用本地 fixture 检索数据；Evidence、checkpoint 和 evaluation 结果通过 SQLite 存储；CLI demo 和 unittest 套件已在本地 `.venv` 验证通过。
...
- Streamlit：`>=1.35`。
- LangGraph：`>=0.2.50`，列在依赖中；当前工作流代码未直接使用 LangGraph 图执行。
- LiteLLM：`>=1.40`，列在依赖中；当前代码未发现实际 LiteLLM 调用或统一 LLM 封装。
...
```bash
PYTHONPATH=src PYTHONDONTWRITEBYTECODE=1 DEEPRESEARCH_SEARCH_PROVIDER=fixture DEEPRESEARCH_STORAGE_PATH=_collab/001_repo-audit-and-agents-draft/runtime/test_research.db .venv/bin/python -m unittest discover -s tests
```

已验证可用的确定性 demo 命令：

```bash
PYTHONPATH=src PYTHONDONTWRITEBYTECODE=1 DEEPRESEARCH_STORAGE_PATH=_collab/001_repo-audit-and-agents-draft/runtime/research.db .venv/bin/python scripts/run_demo.py --topic 'AI Agent 在财富管理行业的落地机会研究' --depth 1 --output _collab/001_repo-audit-and-agents-draft/runtime/demo_report.md
```

未在 001 任务验证的命令：`scripts/run_eval.py`、`scripts/run_checkpoint_demo.py`、`scripts/dev_server.py`、FastAPI/Uvicorn 入口、Streamlit 入口、Docker/Compose 入口。
