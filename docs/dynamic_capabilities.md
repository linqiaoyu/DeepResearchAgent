...
`DYNAMIC_CAPABILITY_ENABLED` 默认开启。规划完成的每个子问题先由确定性分类器
...
关闭开关、没有匹配规则或规则没有可用能力时，选择器显式回退到
`web_search / web_fetch / structured_data_provider` 固定集合，并在
`AgentDecision` 中标记 `fallback=true`，不会静默改变执行路径。

Round 033 的真实双案例消融证明选择器确实改变工具路由，但启用与关闭均为 6/6 正确指标、
0 个幻觉数字，没有测得质量增益。因此默认开启是为了跨问题类型路由与可审计选择，不是
“动态工具优于固定工具”的质量声明；fixture 仍只证明规则、回退和接线按定义执行。
