...
| Decision auditability | Partial | Immutable `DecisionContext` connects budget, sufficiency, prior classifications and Critic issues; decisions have trace, manifest and reader-visible chain landing points | Policies default off and have no real-LLM quality validation |
| Numeric consistency | Partial | Critic deterministically checks growth, share, sum and unit conversions; failures carry claimed/calculated values, formula and Evidence IDs into retry | `NUMERIC_CHECK_ENABLED=false`; real extraction scope/tolerance quality remains unvalidated |
...
| Capability registry | Partial beyond fixed resolution | Search, fetch, structured provider, discovered MCP tools, and loaded skill capabilities share one registry; optional deterministic selector records candidate/selected/rejected/fallback | `DYNAMIC_CAPABILITY_ENABLED=false` and `SKILL_PACKS_ENABLED=false`; no real-mode quality evidence yet |
| Trajectory replay | Partial | `TRAJECTORY_RECORD_ENABLED=false`; expanded fixture trajectory covers LLM/search/fetch/structured/node/decision/signal/memory surfaces and strict report replay | Record the preregistered authorized real superset trajectory in 019 and validate retention/LLM replay |
...
5. Keep branch budget, multi-round research, prior-memory, decision weaving, numeric checking and dynamic capability selection dark until their content and cost effects are measured in comparable real-mode runs.
