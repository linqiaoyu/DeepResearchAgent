...
| Decision auditability | Partial | Loop stop, budget allocation, prior classification and deterministic replan decisions have trace, manifest and report landing points | Policies default off and have no real-LLM quality validation; arithmetic and skill selection remain absent |
...
| Capability registry | Done for fixed resolution | Search, fetch and structured provider carry metadata plus ToolSpec; Researcher receives all three through the registry | Dynamic selection belongs to 016; skill capability registration belongs to 017 |
...
5. Keep branch budget, multi-round research and prior-memory behavior dark until their content and cost effects are measured in comparable real-mode runs.
